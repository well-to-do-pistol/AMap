 ...
 * @since V2.1.0
 */
public class WalkRouteOverlay extends RouteOverlay { //继承RouteOverlay类
     ...
     * @since V2.1.0
     */
    public void addToMap() { //将工具类中的walkPath添加到mMap中

        initPolylineOptions(); //折线基本配置
        try {
            ...
            for (int i = 0; i < walkPaths.size(); i++) {
                ...
                addWalkStationMarkers(walkStep, latLng); //添加节点
                addWalkPolyLines(walkStep); //根据walkStep添加折线

            }
            addStartAndEndMarker(); //往mMap简单添加起终点
    ...
    private void checkDistanceToNextStep(WalkStep walkStep,
        ...
        if (!(lastPoint.equals(nextFirstPoint))) {
            addWalkPolyLine(lastPoint, nextFirstPoint); //有空隙就添加折线
        }
    }
     ...
     * @return
     */
    private LatLonPoint getLastWalkPoint(WalkStep walkStep) { //从Polyline拿到最后一个点
     ...
     * @return
     */
    private LatLonPoint getFirstWalkPoint(WalkStep walkStep) { //从Polyline拿到第一个点
        return walkStep.getPolyline().get(0);
    }


    private void addWalkPolyLine(LatLonPoint pointFrom, LatLonPoint pointTo) { //添加路线(根据LatLonPoint)
        addWalkPolyLine(AMapServicesUtil.convertToLatLng(pointFrom), AMapServicesUtil.convertToLatLng(pointTo));
    }

    private void addWalkPolyLine(LatLng latLngFrom, LatLng latLngTo) { //添加路线(根据LatLng)
     ...
     * @param walkStep
     */
    private void addWalkPolyLines(WalkStep walkStep) { //添加路线(根据步点==LatLng列表?)
        mPolylineOptions.addAll(AMapServicesUtil.convertArrList(walkStep.getPolyline())); //将LatLonPoint列表转化成LatLng列表
    }
    ...
    private void initPolylineOptions() {
        ...
        mPolylineOptions.color(getWalkColor()).width(getRouteWidth()); //设置颜色和宽度
    }


    private void showPolyline() { //添加折线
