...
public class RideRouteOverlay extends RouteOverlay {
    ...
    public void addToMap() {
        ...
        initPolylineOptions();
        try {
            List<RideStep> ridePaths = ridePath.getSteps(); //ridePaths拿rideStep, rideStep又拿Polyline, Polyline又拿latLng
            for (int i = 0; i < ridePaths.size(); i++) {
                RideStep rideStep = ridePaths.get(i);
                LatLng latLng = MapUtil.convertToLatLng(rideStep
                        .getPolyline().get(0));

                addRideStationMarkers(rideStep, latLng); //添加节点
                addRidePolyLines(rideStep); //根据rideStep添加折线
            }
    ...
    private void initPolylineOptions() {
        ...
        mPolylineOptions.color(getDriveColor()).width(getRouteWidth()); //配置颜色和宽度
    }
